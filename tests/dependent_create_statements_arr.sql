...
SELECT is(
    dependent_create_statements_arr(
        table_ref('public', 'data')
    ),
    ARRAY[
        'CREATE VIEW public.data_ex1 AS  SELECT (data.x + 1) AS x,
    (data.y * (2)::double precision) AS y
   FROM data;',
        'ALTER VIEW public.data_ex1 OWNER TO vagrant;',
